...
public class KasiinoMängGUI extends Application {
    ...
    public void start(Stage stage) {
        ...
        slotmachineNupp.setOnMouseClicked(event3 -> {
            SlotGUI uus = null;
            try {
                uus = new SlotGUI(stseen);
            } catch (FileNotFoundException e) {
                e.printStackTrace();
            }
            Group SlotJuur = uus.start();
            SlotJuur.getChildren().add(exitNupp);
            stseen.setRoot(SlotJuur);

        });
        playNupp.setOnMouseClicked(event -> {
            juur.getChildren().clear(); //kui play nuppu vajutada, tühjendab juure
